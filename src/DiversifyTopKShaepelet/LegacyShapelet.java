...
//                this.informationGain = bsfGain;
    ...
    public boolean isSimilar(LegacyShapelet shapelet, Instances data ){
        ...
        double dist=0;
        if(this.content.length>shapelet.content.length)
            dist=DiversifyTopKShaepelet.subsequenceDistance(shapelet.content, this.content);
        else
            dist=DiversifyTopKShaepelet.subsequenceDistance(this.content, shapelet.content);
